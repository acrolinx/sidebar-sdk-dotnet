...
namespace Acrolinx.Sdk.Sidebar
{
    ...
    public class AcrolinxPlugin
    {
        ...
        public void OnAfterObjectSet()
        {
            ...
            webBrowser.Document.InvokeScript("eval", new object[] { "window.acrolinxStorage = { getItem: function(key) { return window.external.getItem(key); }, removeItem: function(key) { window.external.removeItem(key); }, setItem: function(key, data) { window.external.setItem(key, data); }, clear: function() { window.external.clear(); } }" });
        ...
        public void clear(params dynamic[] o)
        {
            Logger.AcroLog.Debug("clear");

            sidebar.Storage.Clear();
        }
